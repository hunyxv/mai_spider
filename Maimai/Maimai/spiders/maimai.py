...
Sql.push_new_encode_mmid('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1IjozMzU2MjU3NiwibGV2ZWwiOjEsInQiOiJjdHQifQ.qdr66B4BpFJLY4PGKLOaceD7mYcc-BT5mtuXe3s30yM', 202309)
# COOKIE
...
class MaimaiSpider(scrapy.Spider):
    ...
    referer = 'https://maimai.cn/contact/detail/{}?from=webview%23%2Ffeed_list'
    ...
    def get_info(self, response):
        ...
        try:
            ...
            for work_exp in uinfo['work_exp']:
                work = {}
                work['id'] = id
                work['company'] = work_exp['company']
                work['position'] = work_exp['position']
                work['description'] = work_exp.get('description', '')
                work['start_date'] = work_exp['start_date']
                work['end_date'] = WORK_END_DATE(work_exp['end_date'])
                item['work_exp'].append(work)
                # 获取他(她)在本公司的好友
                cookies = random.choice(COOKIES)
                friends_list_url = self.friends_url.format(work['company'], item['id'])
            ...
            for edu_exp in uinfo['education']:
                edu = {}
                edu['id'] = id
                edu['school'] = edu_exp['school']
                edu['degree'] = DEGREE_DICT[edu_exp.get('degree', '255')]
                edu['department'] = edu_exp['department']
                edu['start_date'] = edu_exp['start_date']
                edu['end_date'] = edu_exp.get('end_date', '')
                item['education'].append(edu)
                # 获取ta校友列表
                cookies = random.choice(COOKIES)
                friends_list_url = self.school_url.format(edu['school'], item['id'])
                yield scrapy.Request(friends_list_url,
                                     callback=self.get_encode_mmid,
                                     cookies=cookies,
                                     headers={'Referer': 'https://maimai.cn/'})
    ...
    def get_comment(self, response):
        ...
        try:
            ...
            for comment in comment_list:
                comment = {}
                comment['id'] = comment['user']['id']
                comment['friend_id'] = comment['src_user']['id']
                comment['friend_name'] = comment['src_user']['name']
                comment['friend_company'] = comment['src_user']['company']
                comment['friend_position'] = comment['src_user']['position']
                comment['level'] = comment['re']
